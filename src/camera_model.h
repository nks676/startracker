...
// so callers that only set focal/center fields continue to work.
//
// Phase 3g.1: extracted from identification.h so calibration (tools/
// calibrate_camera.py output) and tracking (3d) can depend on the camera
// concept without pulling in the identification pipeline.
struct CameraModel {
  double frame_width;
  double frame_height;
  double focal_x;
